// Black Hole Visualization Effect - Full CodePen Integration
// Complete Three.js black hole with shaders, lensing, and effects

export function showBlackHole(duration = 0) {
	...
	container.id = 'blackhole-container-instance';
...
<div id="info">
	...
	// Load Three.js using import map defined in index.html
	Promise.all([
		import('three'),
	...
	]).then(([THREE, OrbitControlsModule, EffectComposerModule, RenderPassModule, UnrealBloomPassModule, ShaderPassModule]) => {
		...
		const camera = new THREE.PerspectiveCamera(60, containerRect.width / containerRect.height, 0.1, 4000);
		...
		const lensingShader = {
			uniforms: {
				...
				"aspectRatio": { value: containerRect.width / containerRect.height },
		...
		// Accretion disk with full shader
		...
		// Hide info text after 5 seconds
		setTimeout(() => {
		...
		container.innerHTML = `<div style="color: white; padding: 20px; text-align: center;">
			Failed to load visualization.<br>
